...
- Honor an explicit user request for Ultracode with `ultracode: true`,
  `confirmUltracode: true`, and no ordinary effort.
...
7. **Synthesize.** Lead with verified issues ordered by severity, then testing
   gaps and residual risk. State the requested, resolved, and observed Claude
   posture accurately.
8. **Clean up.** Recheck repository/index state, then gracefully stop the review
   session created by this workflow unless follow-up continuity is requested.
